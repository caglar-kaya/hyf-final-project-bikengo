  ...
  const performFetch = (options) => {
    ...
    const fetchUsers = async () => {
      ...
      if (jsonResult.success === true) {
        ...
        jsonResult.msg === "Invalid token" ||
        jsonResult.msg === "Access Denied"
      ) {
        setError("Something went wrong, please Login or register.");
      } else if (jsonResult.msg.includes("BAD REQUEST:")) {
        setError(jsonResult.msg.split("BAD REQUEST:")[1]);
      } else {
        setError(
          jsonResult.msg ||
